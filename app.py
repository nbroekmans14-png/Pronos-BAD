...
if is_locked:
    ...
    if nom_input:
        ...
        st.write("Quel sera le score global de la rencontre ?")
        c1, c2 = st.columns(2)
...
with st.expander("🛠️ Administration"):
    ...
    if mdp == "2003":
        ...
        with t1:
            ...
            st.subheader("2. Score Final de la rencontre")
            ...
            if st.button("Calculer et Valider la journée"):
                ...
                if not df_v.empty:
                    ...
                    st.success(f"Journée validée ! Score final retenu : {score_final_reel}")
                    st.rerun()
        ...
        with t5:
            st.subheader("1. Restaurer le Classement")
            file_score = st.file_uploader("Charger classement_general_backup.csv", type="csv")
            if file_score and st.button("Restaurer CLASSEMENT"):
                save_df(pd.read_csv(file_score), SCORES_FILE); st.rerun()
            st.divider()
            st.subheader("2. Restaurer les Votes")
            file_votes = st.file_uploader("Charger backup_votes.csv", type="csv")
            if file_votes and st.button("Restaurer VOTES"):
                save_df(pd.read_csv(file_votes), VOTES_FILE); st.rerun()
